...
Blargg's cpu_instrs test 02 (interrupts) was failing while 01 and 03–11 passed. The tool compares your emulator's per-instruction trace log against a known-good reference log, line by line, and flags the first line where they diverge. That's the key tool here: it doesn't just say "wrong," it gives you the exact CPU state at the exact instruction where reality and your emulator first disagree — which turns "why is my interrupt handling broken" into "why does line 151346 show PC:C2C0 in the reference but PC:0050 in mine," a much more tractable question.

Bug 1 — the dispatch was silently unlogged
CPU::step() had handleInterrupts() return early (return 20) before the line that logs CPU state. So whenever an interrupt actually fired, that step produced zero log lines instead of one. Every line after that point was off by one relative to the reference, which looks exactly like a garbled trace even though the interrupt itself dispatched correctly. First lesson: a missing log line and a wrong CPU state produce the identical symptom — you have to check whether you're comparing the same moment in time before you conclude the logic is wrong.
